...
if [ "$1" == "neo4j" ]; then
    ...
    setting "dbms.pagecache.memory" "${NEO4J_CACHE_MEMORY:-2048M}" neo4j.properties
    setting "wrapper.java.additional=-Dneo4j.ext.udc.source" "${NEO4J_UDC_SOURCE:-docker}" neo4j-wrapper.conf
    setting "wrapper.java.initmemory" "${NEO4J_HEAP_MEMORY:-8192}" neo4j-wrapper.conf
    setting "wrapper.java.maxmemory" "${NEO4J_HEAP_MEMORY:-8192}" neo4j-wrapper.conf
